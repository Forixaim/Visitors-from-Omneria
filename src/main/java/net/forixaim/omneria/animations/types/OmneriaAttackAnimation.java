    ...
    protected void hurtCollidingEntities(LivingEntityPatch<?> entitypatch, float prevElapsedTime, float elapsedTime, EntityState prevState, EntityState state, Phase phase) {
        ...
        if (!list.isEmpty()) {
            ...
            while (entitypatch.getCurrentlyActuallyHitEntities().size() < maxStrikes && hitEntities.next())
            {
                ...
                {
                    if (target instanceof GuardianCrystalEntity entity)
                    {
                        if (entitypatch instanceof ServerPlayerPatch patch)
                        {
                            SkillContainer bs = patch.getSkill(BattleArtsSkillSlots.BATTLE_STYLE);
                            if (bs.hasSkill(OmneriaSkills.GENESIS_WYRM))
                            {
                                entity.destabilize();
                                entity.setShieldPower(0);
                                patch.attack(this.getEpicFightDamageSource(entitypatch, target, phase), target, phase.hand);
                            }
                        }
                    }
                }
